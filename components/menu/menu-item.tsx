...
function MenuItem(props: MenuItemProps) {
          ...
          return (
            <Fragment>
              <Link
                href="#"
                className={cn(
                  'group relative flex items-center justify-between rounded-sm px-4 py-2 duration-300 ease-in-out hover:bg-primary/10',
                )}
                onClick={(e) => {
                  e.preventDefault();
                  handleClick();
  ...
  return (
    <Link
      href={to ?? '#'}
      className={cn(
        'group relative flex items-center gap-2.5 rounded-sm px-4 py-2 duration-300 ease-in-out hover:bg-primary/10',
        {
          'bg-primary/10 text-primary font-medium': pathname?.includes(itemKey),
        },
      )}
      onClick={(e) => {
